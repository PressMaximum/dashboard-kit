...
```json
{
  ...
  "exports": {
    ...
    "./datasets": {
      ...
      "types": "./build/datasets/index.d.ts"
    }
  }
}
```
...
│   ├── createI18nBag.js               // returns object proxy that throws if accessed key not in bag
│   └── createUid.js                   // 8-char hex helper (reused from Blocksify's helpers.js)
├── layouts/
│   ├── ListPageHeader/
│   │   ├── index.js
│   │   └── editor.css
...
| `.pmdk-schema-form` | Settings form container |
| `.pmdk-schema-field` | Single field row |
...
Classes are part of the public API — locked at 1.0, deprecation cycle for changes.
